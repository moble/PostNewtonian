                               ...
                               )
{
  ...
  const Quaternion S_S1_i = ( Quaternions::Quaternion(chi1_i).abs()>1e-10
                              ? Quaternions::Quaternion(chi1_i).abs()*Quaternions::sqrtOfRotor(-Quaternions::Quaternion(chi1_i).normalized()*Quaternions::zHat)
                              : Quaternions::Zero);
  const Quaternion S_S2_i = ( Quaternions::Quaternion(chi2_i).abs()>1e-10
                              ? Quaternions::Quaternion(chi2_i).abs()*Quaternions::sqrtOfRotor(-Quaternions::Quaternion(chi2_i).normalized()*Quaternions::zHat)
                              : Quaternions::Zero);
  ...
  // Store the data at the first step
  {
    ...
    chi1.push_back((S_S1_i*R_chi1_i*zHat*R_chi1_i.conjugate()*S_S1_i.conjugate()).vec());
    const Quaternion R_chi2_i = exp(Quaternion(0.0, y[3], y[4], 0.0));
    chi2.push_back((S_S2_i*R_chi2_i*zHat*R_chi2_i.conjugate()*S_S2_i.conjugate()).vec());
